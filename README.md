...
4. 「種類の選択」で「ウェブアプリ」を選び、以下を設定してデプロイ
   - 実行するユーザー: 自分
   - **アクセスできるユーザー: 全員**（ここが「自分のみ」や「Googleアカウントを持つ全員」だと、サイトからの記録が弾かれます。必ず「全員」にしてください）
...
### スクリプトを後から書き換えたときの注意（重要）

`google-apps-script.gs` の内容を変更したら、必ず**新しいバージョンで再デプロイ**してください。
保存しただけでは、`/exec` のURLは古いコードのまま動きます。

1. Apps Script エディタ右上「デプロイ」→「デプロイを管理」
2. 対象デプロイの鉛筆（編集）アイコンをクリック
3. 「バージョン」を **「新バージョン」** に変更して「デプロイ」

URL自体は変わらないので、`FORM_ENDPOINT` の貼り替えは不要です。

> confirm.html は通常のフォーム送信（`application/x-www-form-urlencoded`）で記録先へ送信します。
> CORS・プリフライト・リダイレクトの影響を受けにくく、Apps Script の Web アプリに確実に届く方式です。
